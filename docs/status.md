...
## 侧栏切换死循环与辅助功能遍历开销（2026-09-15）

用户报告点击活动窗口的「隐藏侧栏」按钮后应用一直转圈。本机复现：辅助功能与真实坐标点击均可触发，主线程 100% CPU，采样停在 NSWindow 布局循环，最终 AppKit 抛出「more Update Constraints in Window passes than there are views」。公开发布的 preview.2 同样复现。对照实验逐一排除了工具栏宽度测量、窗口宽度驱动的检查器收起、检查器固定宽度、分栏样式；把 `.inspector` 从 detail 列挪到 NavigationSplitView 本身后，检查器可见时切换侧栏三次均正常，CPU 归零。

同时把原生表格的辅助功能子元素限制为可见行：AppKit 回答整表子元素时会为每行每列创建单元格视图，189 条记录一次遍历 2.5 秒且占用主线程，任何常驻的辅助功能客户端轮询都会让应用长期无响应；改后一次遍历约 0.04 秒。

验证：175 项 Swift 测试通过；release 打包与 deep/strict 签名校验通过；实机切换侧栏与检查器各多次无卡顿。未覆盖：VoiceOver 对不可见行的朗读体验。
